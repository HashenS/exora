...
void main() {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const targetElement = interactiveTargetRef?.current || containerRef.current;
    ...
    const eventTarget = interactiveTargetRef?.current || window;
    const hoverTarget = interactiveTargetRef?.current || containerRef.current;

    if (mouseInteraction) {
      eventTarget.addEventListener('mousemove', handleMouseMove as any);
      if (hoverTarget) {
    ...
    return () => {
      ...
      if (mouseInteraction) {
        eventTarget.removeEventListener('mousemove', handleMouseMove as any);
        if (hoverTarget) {
  ...
  return <div ref={containerRef} className="ripple-grid-container" style={{ width: '100%', height: '100%' }} />;
};
